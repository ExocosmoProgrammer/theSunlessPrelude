...
class player:
        ...
        if enemies:
            ...
            try:
                if enemy.type in ['alien priest', 'sun priest', 'alien bishop'] and [foe for foe in enemies if
                    ...
                    printWithPause(0.5, '\033[35m', f'You hit {enemy.getPrintName()}, but they were '
                                                    f'immune.')

                else:
                    if percentChance(50):
                        damageInflicted = getReducedDamage(self.attack, enemy)
                        printWithPause(0.5, '\033[35m', f'You hit {enemy.getPrintName()}, inflicting '
                                                        f'{damageInflicted} '
                                                        f'damage.')

                    else:
                        damageInflicted = getReducedDamage(self.attack, enemy) * 2
                        printWithPause(0.5, '\033[35m', f'You hit {enemy.getPrintName()} with a critical '
                    ...
                    if enemy.gunWeakness and 'gun' in self.inventory:
                        if enemy.type == 'alien commander' and [protector for protector in
                                                                ...
                                                                == 'alien protector']:
                            printWithPause(0.5, '\033[35m', f'You shot {enemy.getPrintName()}, but they '
                                                            f'were immune.')
                            ...
                            if enemy.twirlingNunchucks:
                                ...
                                for i in range(self.inventory.count('gun')):
                                    printWithPause(0.5, '\033[35m', f'You shot {enemy.getPrintName()}, '
        ...
        try:
            if enemy.type in ['alien priest', 'sun priest', 'alien bishop'] and [foe for foe in enemies if
                ...
                attackedFoe = random.choice([foe for foe in enemies if
                                             ...
                                             enemy.possessed])
                attackedFoe.hp = 0
                printWithPause(0.5, '\033[35m', f'{attackedFoe.getPrintName()} got in the way of your '
                                                f'attack.')

            elif enemy.type == 'alien commander' and [foe for foe in enemies if
                                                      foe.type == 'alien protector']:
                printWithPause(0.5, '\033[35m', f'You stabbed {enemy.getPrintName()}, but they were '
                                                f'immune.')

            else:
                damageInflicted = getReducedDamage(self.attack / 2, enemy)
                printWithPause(0.5, '\033[35m', f'You stabbed {enemy.getPrintName()} '
                ...
                if enemy.type in ['alien priest', 'sun priest', 'alien bishop'] and [foe for foe in enemies if
                    ...
                    printWithPause(0.5, '\033[35m', f'{attackedFoe.getPrintName()} got in the way of '
                                                    f'your attack.')
                                                          ...
                                                          foe.type == 'alien protector']:
                    printWithPause(0.5, '\033[35m', f'You slashed {enemy.getPrintName()}, but they '
                                                    f'were immune.')

                else:
                    damageInflicted = getReducedDamage(self.attack * 3 / 8, enemy)
                    printWithPause(0.5, '\033[35m', f'You slashed {enemy.getPrintName()} '
    ...
    def useNunchucks(self, enemies):
        ...
        try:
            if enemy.type in ['alien priest', 'sun priest', 'alien bishop'] and [foe for foe in enemies if
                ...
                printWithPause(0.5, '\033[35m', f'You hit {enemy.getPrintName()}, but they were immune.')

            else:
                damageInflicted = getReducedDamage(10, enemy)
                printWithPause(0.5, '\033[35m', f'You hit {enemy.getPrintName()} '
            ...
            try:
                ...
                if enemy.type in ['alien priest', 'sun priest', 'alien bishop'] and [foe for foe in enemies if
                    ...
                    attackedFoe = random.choice([foe for foe in enemies if
                                                 ...
                                                 enemy.possessed])
                    attackedFoe.hp = 0
                    printWithPause(0.5, '\033[35m', f'{attackedFoe.getPrintName()} got in the way '
                                                    f'of your knife.')

                else:
                    printWithPause(0.5, '\033[35m', f'You hit {enemy.getPrintName()} with your '
    ...
    def useVialOfPoison(self, enemies):
        ...
        if 'vial of poison' in self.inventory:
            ...
            try:
                if enemy.type == 'alien nun' and enemy.twirlingNunchucks:
                    printWithPause(0.5, '\033[35m', f'{enemy.getPrintName()} hit your vial of poison '
                    ...
                    printWithPause(0.5, '\033[35m', f'You hit {enemy.getPrintName()} with your vial, '
                                                    f'poisoning them.')
                    enemy.poisonDamage = 15
        ...
        if 'serrated knife' in self.inventory:
            ...
            try:
                ...
                if enemy.type in ['alien priest', 'sun priest', 'alien bishop'] and [foe for foe in enemies if
                    ...
                    printWithPause(0.5, '\033[35m', f'You hit {enemy.getPrintName()} with your knife, '
    ...
    def actions(self, enemies, level):
        ...
        if self.stun:
            ...
            if not [enemy for enemy in enemies if enemy.type == 'helpless sun priest']:
                ...
                if enemies[0].hp == 40:
                    printWithPause(1, '\033[35m', 'You hit the sun priest. You hear something crack.')

                elif enemies[0].hp == 30:
                    printWithPause(1, '\033[35m', 'You hit the sun priest. Blood stains your fists.')

                elif enemies[0].hp == 20:
                    printWithPause(1, '\033[35m', 'You hit the sun priest. Its face is disfigured '
                                                  'and broken.')

                elif enemies[0].hp == 10:
                    printWithPause(1, '\033[35m', 'You hit the sun priest. Something squelches.')

                elif enemies[0].hp == 0:
                    printWithPause(1, '\033[35m', 'You hit the sun priest. You hear nothing but '
