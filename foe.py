...
class foe:
    ...
    def actAsAlienGeneral(self, target, number, enemies, player):
        ...
        if ((not self.possessed or len(otherFoes) > 1) and (not self.stun and percentChance(33)) and
                self.recentSummonQty < 4 and not \
                        (self.possessed and len([enemy for enemy in enemies if enemy.type == 'drone']) > 4 * \
                         len([enemy for enemy in enemies if enemy.type == 'alien general']))):
        ...
        if not self.stun:
            if not self.preparingGrenade and percentChance(50):
                ...
                if not self.possessed:
                    ...
                    self.hp -= 5
                    printWithPause(0.5, '\033[93m', f'{self.getPrintName()} took 5 damage from their '
                                                    f'stun grenade.')
            ...
            if self.cooldown == 1:
                ...
                if self.nextAttack == 'summon':
                    ...
                    for enemy in [enemy for enemy in enemies if enemy.type != 'sun priest']:
                        ...
                        printWithPause(0.5, '\033[93m', f'{enemy.getPrintName()} was set on fire')
