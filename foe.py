...
hpPerFoe = {'alien colonist': 10, 'alien bodyguard': 20, 'alien police': 30,
            ...
            'alien cultist': 30, 'alien nun': 50, 'sun priest': 3000}
...
lootPerFoe = {'alien colonist': {'regen': 8}, 'alien bodyguard': {'shield': 8},
              'alien police': {'baton': 8}, 'alien soldier': {'knife': 8},
              'alien secret service': {'gun': 8}, 'alien assassin': {'stun grenade': 100, 'gas canister': 8},
              'alien minion': {'throwing knife': 100, 'radio': 8},
              'alien scientist': {'solution': 100, 'hissing cockroach': 8, 'combustible lemon': 1},
              'alien general': {'sword': 50, 'drone': 8},
              'alien henchman': {'armor': 50, 'serrated knife': 100, 'vial of diseased blood': 8},
              'alien combat scientist': {'solution': 100, 'hissing cockroach': 8, 'combustible lemon': 1},
              'alien doctor': {'solution': 50, 'throwing knife': 50},
              'alien cultist': {'vial of poison': 100, 'sacrificial dagger': 8}, 'alien nun': {'nunchucks': 8},
              'alien priest': {'cross': 8}, 'alien gangster': {'butterfly knife': 8}}


class foe:
    def __init__(self, name, number, scanned=0, givesPotions=1, possessed=0, loot=None, controlledByPro=0):
        self.type = name
        self.number = number
    ...
    def actAsAlienCommander(self, target, number, enemies):
        ...
        if [enemy for enemy in enemies if enemy.type == 'alien protector']:
            ...
            if self.poisonDamage:
                printWithPause(f"{self.getPrintName()} is no longer poisoned.")
                self.poisonDamage = 0
    ...
    def empowerEnemyAsScientist(self, enemies):
        try:
            if self.possessed:
                if self.controlledByPro and [enemy for enemy in enemies if
                                             not enemy.possessed]:
                    targetId = input(f"Who shall {self.getPrintName()} weaken? Type the foe's id or 'r' to "
                                     f"weaken a random foe:")
                    foeWeakened = getTarget(enemies, targetId)
    ...
    def actAsAlienDoctor(self, target, number, enemies):
        if not self.stun:
            if not self.possessed and percentChance(50):
                for enemy in enemies:
                ...
                warrior = [enemy for enemy in enemies if enemy.type in ['alien warrior', 'sun priest']][0]
                self.basicAttack(warrior, number, enemies)
    ...
    def getDamage(self, enemies):
        if self.type == 'alien cultist':
            self.attack = self.standardAttack * len([enemy for enemy in enemies if
                                                     enemy.type == 'alien cultist' and
                                                     enemy.possessed == self.possessed])
    ...
    def getUpdate(self):
        self.hp = hpPerFoe[self.type]
        self.attack = attackPerFoe[self.type]
        self.loot = lootPerFoe[self.type]
