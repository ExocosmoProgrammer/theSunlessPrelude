...
from variables import bossesPerLevel, oneTimeUseItems, hpPerFoe, attackPerFoe, lootPerFoe
from definitions import printWithPause, getReducedDamage, greater, percentChance, getTarget, getInput, lesser, play
...
class foe:
    def __init__(self, name, number, scanned=0, givesPotions=1, possessed=0, loot=None,
        ...
        foeAttackMethods = {'alien commander': self.actAsAlienCommander,
                            ...
                            'the sun': self.actAsTheSun,
                            'alien mother superior': self.actAsAlienMotherSuperior,
                            'alien devotee': self.actAsAlienDevotee,
                            'helpless sun priest': self.actAsHelplessSunPriest
                            }
        ...
        if self.controlledByPro and [enemy for enemy in enemies if not enemy.possessed and
            ...
            try:
                if self.strengthened:
                    ...
                    nonReducedDamage = self.attack * 2

                else:
                    damageTaken = getReducedDamage(self.attack, target)
                    nonReducedDamage = self.attack

                if target.type == 'alien commander' and [enemy for enemy in enemies if
                                                         enemy.type == 'alien protector']:
                    printWithPause(0.5, '\033[35m', f'{self.getPrintName()} hit '
                                                    f'{target.getPrintName()}, but they were immune.')

                elif target.type in ['alien priest', 'sun priest', 'alien bishop'] and [enemy for enemy in enemies if \
                                                                        enemy.type in ['alien worshipper',
                                                                                       'alien cardinal'] and \
                                                                        enemy.hp > 0 and \
                                                                        enemy.possessed == target.possessed]:
                    attackedFoe = random.choice([enemy for enemy in enemies if
                                                 ...
                                                 target.possessed])
                    attackedFoe.hp = 0
                    printWithPause(0.5, '\033[35m', f"{attackedFoe.getPrintName()} got in the way of "
                                                    f"{self.getPrintName()}'s attack.")

                else:
                    printWithPause(0.5, '\033[35m', f'{self.getPrintName()} attacked '
                    ...
                    if self.poisonAttack:
                        ...
                        printWithPause(0.5, '\033[35m', f'{self.getPrintName()} poisoned '
                                                        f'{target.getPrintName()}')
                        target.turnsOfPoisonDamage = 10

                    if target.twirlingNunchucks:
                        self.hp -= nonReducedDamage
                        printWithPause(0.5, '\033[35m', f'{target.fullName} deflected {self.fullName}\'s '
            ...
            if self.strengthened:
                ...
                if self.possessed:
                    printWithPause(0.5, '\033[35m', f'{self.getPrintName()} hit {target.getPrintName()}, '
                    ...
                    if target.isDrone:
                        printWithPause(0.5, '\033[91m', f'Your drone was hit by '
            ...
            if random.randint(0, 1):
                ...
                if target.foe:
                    if target.type in ['alien priest', 'sun priest', 'alien bishop'] and \
                            [enemy for enemy in enemies if enemy.type in ['alien worshipper', 'alien cardinal'] and \
                                                           enemy.possessed == target.possessed]:
                        target = random.choice([enemy for enemy in enemies if
                                                enemy.type in ['alien worshipper', 'alien cardinal']])
                        printWithPause(0.5, '\033[35m', f"{target.getPrintName()} blocked "
                                                        f"{self.getPrintName()}'s attack.")
                        ...
                        if target.twirlingNunchucks:
                            self.hp -= nonReducedDamage
                            printWithPause(0.5, '\033[35m', f'{target.fullName} reflected '
                                                            f'{self.getPrintName()}\'s attack, inflicting '
                                                            f'{damageTaken} damage to {self.fullName}.')

                elif not self.scaredOfCockroach:
                    if self.type in ['alien gangster', 'alien mobster']:
                        if not target.isDrone and target.blocking:
                            ...
                            if target.isDrone:
                                printWithPause(0.5, '\033[91m', f'{self.getPrintName()} hit your drone, '
                        ...
                        if self.nunchuckDebuff:
                            ...
                            printWithPause(0.5, '\033[35m', f"{self.getPrintName()}'s attack was "
                                                            f"deflected by your nunchucks, inflicting "
                                                            f"{damageTaken * 4} damage to the foe.")

                            if self.twirlingNunchucks:
                                target.hp -= damageTaken
                                printWithPause(0.5, '\033[91m', f"{self.fullName} deflected your "
                                                                f"deflection, inflicting {damageTaken} damage to you.")

                    else:
                        printWithPause(0.5, '\033[91m', f'{self.getPrintName()} attacked your drone, '
                                                        f'inflicting {damageTaken} damage.')
                ...
                if damageTaken is not None:
                    ...
                    if self.poisonAttack:
                        ...
                        if target.foe:
                            ...
                            printWithPause(0.5, '\033[91m', f'{self.getPrintName()} poisoned your '
                                                            f'drone.')

                        else:
                            printWithPause(0.5, '\033[91m', f'{self.getPrintName()} poisoned you.')
    ...
    def getHurtByDebuffs(self, protagonist):
        ...
        if self.bleedingDamage:
            ...
            printWithPause(0.5, '\033[35m', f'{self.getPrintName()} took {self.bleedingDamage} damage '
                                            f'from bleeding.')

            if 'vial of diseased blood' in protagonist.inventory:
                self.hp -= 3
                printWithPause(0.5, '\033[35m', f'{self.getPrintName()} took 3 damage from the '
                                                f'disease in their blood.')

        if self.bleedingDamageFromGun:
            self.hp -= self.bleedingDamageFromGun
            printWithPause(0.5, '\033[35m', f'{self.getPrintName()} took {self.bleedingDamageFromGun} '
        ...
        if self.poisonDamage:
            ...
            printWithPause(0.5, '\033[35m', f'{self.getPrintName()} took {self.poisonDamage} damage '
                                            f'from poison.')

        if self.burnDamage:
            self.hp -= self.burnDamage
            printWithPause(0.5, '\033[35m', f'{self.getPrintName()} took {self.burnDamage} from burning.')
    ...
    def actAsIdol(self, target, number, enemies, player):
        ...
        if self.possessed:
            printWithPause(0.5, '\033[35m', f"{target.getPrintName()} is hurt by an idol's presence, "
                                            f"inflicting 5 damage.")

        else:
            printWithPause(0.5, '\033[91m', f"You are hurt by an idol's presence, inflicting 5 damage.")
    ...
    def empowerEnemyAsScientist(self, enemies):
        try:
            if self.possessed:
                ...
                enemy.strengthened and not enemy.possessed and enemy.type != 'the sun'])
        ...
        if not self.stun:
            if not self.preparingGrenade and percentChance(50):
                ...
                if not self.possessed:
                    ...
                    self.hp -= 5
                    printWithPause(0.5, '\033[35m', f'{self.getPrintName()} took 5 damage from their '
                                                    f'stun grenade.')
    ...
    def actAsAlienPriest(self, target, number, enemies, player):
        if not self.stun:
            if (percentChance(33) and len([enemy for enemy in enemies if
                ...
                self.newFoes.append(foe('alien worshipper', number, scanned=1, possessed=self.possessed))
                number += 1
    ...
    def actAsAlienBishop(self, target, number, enemies, player):
        if not self.stun:
            if (percentChance(33) and len([enemy for enemy in enemies if
                                           enemy.type in ['alien worshipper', 'alien cardinal']]) <= 1 and
                    self.recentSummonQty < 3):
                self.newFoes.append(foe('alien cardinal', number, scanned=1, possessed=self.possessed))
                number += 1
                printWithPause(0.5, '\033[96m', f'{self.fullName} summoned '
                                                f'{self.newFoes[-1].getPrintName()}.')
                self.recentSummonQty += 1
        ...
        if not self.stun:
            ...
            if self.possessed:
                try:
                    ...
                    printWithPause(0.5, '\033[35m', f'{self.fullName} attacked '
                ...
                printWithPause(0.5, '\033[91m', f'{self.fullName} hit you, inflicting {self.attack} '
                                                f'damage.')
    ...
    def actAsAlienAdmiral(self, target, number, enemies, player):
        ...
        if ((not self.possessed or len(otherFoes) > 1) and (not self.stun and percentChance(33)) and
                self.recentSummonQty < 4):
            self.newFoes.append(foe('stronger drone', number, scanned=1,
                                    possessed=self.possessed, givesPotions=0))
            printWithPause(0.5, '\033[96m', f'{self.fullName} summoned '
    ...
    def actAsAlienMotherSuperior(self, target, number, enemies, player):
        if not self.stun:
            self.cooldown -= 1
            self.twirlingNunchucks = 0
            self.damageReduction = 0
            ...
            if self.cooldown <= 0:
                ...
                printWithPause(0.5, '\033[96m', f'{self.fullName} is twirling their nunchucks quicker '
    ...
    def actAsTheSun(self, target, number, enemies, player):
        ...
        if self.cooldown == 1:
            printWithPause(0.5, '\033[96m', 'The sun is preparing to burn you')

        elif self.cooldown <= 0:
            player.hp -= self.attack
            printWithPause(0.5, '\033[91m', f'You were burned by the sun, inflicting {self.attack} '
                                            f'damage.')
            self.cooldown = 5

    def actAsAlienDevotee(self, target, number, enemies, player):
        self.regeneration = 10 / len([enemy for enemy in enemies if enemy.type in \
                                      ['alien devotee', 'alien cultist']])
        self.basicAttack(target, number, enemies, player)

        if self.hp < 50:
            oldHp = self.hp
            self.hp = lesser(self.hp + self.regeneration, 50)
            printWithPause(0.5, '\033[96m', f'{self.fullName} regenerated {self.hp - oldHp} hit points.')
        ...
        if not self.stun:
            if self.recentSummonQty < 7:
                if self.hp <= 67 and percentChance(33):
                    ...
                    number += 1
                    printWithPause(0.5, '\033[96m', f'{self.getPrintName()} summoned '
                                                    f'{self.newFoes[-1].getPrintName()} to attack you.')
                    self.recentSummonQty += 1
            ...
            if self.hp <= 133 and self.timesSummoned == 0:
                ...
                number += 1
                printWithPause(0.5, '\033[96m', f'{self.getPrintName()} summoned '
                                                f'{self.newFoes[-1].getPrintName()} to protect the commander.')
    ...
    def actAsAlienPilot(self, target, number, enemies, player):
        if not self.stun:
            if ((self.hp > 125 and percentChance(25)) or (self.hp <= 125 and percentChance(50))
                ...
                foeChoices = [enemy for enemy in ['alien assassin', 'alien minion', 'alien scientist',
                              'alien general', 'alien henchman'] if enemy not in self.recentSummonTypes]
                foeSummoned = random.choice(foeChoices)
                self.newFoes.append(foe(foeSummoned, number, scanned=self.scanned, givesPotions=0, isSummon=1))
                printWithPause(0.5, '\033[96m', f'{self.getPrintName()} summoned '
                                                f'{self.newFoes[-1].getPrintName()}.')
                number += 1
                self.recentSummonQty += 1
                self.recentSummonTypes.append(foeSummoned)
        ...
        if not self.stun:
            ...
            if self.cooldown == 1:
                ...
                if self.nextAttack == 'summon':
                    ...
                    player.hp -= damageToPro
                    printWithPause(0.5, '\033[91m', f'You were hit by an explosion, inflicting '
                                                    f'{damageToPro} damage.')

                    for enemy in enemies:
                        enemy.hp -= 40
                        printWithPause(0.5, '\033[35m', f'{enemy.getPrintName()} was hit by an '
    ...
    def actAsSunPriestPt2(self, target, number, enemies, player):
        if self.lastPtAttackMethodUsed == 1:
            ...
            printWithPause(0.5, '\033[95m', 'The sun priest is changing its strategy.')
        ...
        if not self.stun:
            ...
            if self.cooldown == 1:
                ...
                elif self.nextAttack == 'incendiary':
                    printWithPause(0.5, '\033[96m', f'{self.fullName} is preparing to set you on fire.')
                ...
                if self.nextAttack == 'summon':
                    ...
                    printWithPause(0.5, '\033[96m', f'{self.getPrintName()} threw an incendiary.')
                    printWithPause(0.5, '\033[91m', f'You were set on fire.')
                    player.burningDamage = greater(player.burningDamage, 10)
                    player.turnsOfBurningDamage = 10

                    if 'drone' in player.inventory:
                        printWithPause(0.5, '\033[91m', 'Your drone was set on fire.')
                        player.drone.burningDamage = greater(player.drone.burningDamage, 10)
                        player.drone.turnsOfBurningDamage = 10

                    for enemy in [enemy for enemy in enemies if enemy.type != 'sun priest']:
                        enemy.burnDamage = greater(enemy.burnDamage, 10)
                        printWithPause(0.5, '\033[35m', f'{enemy.getPrintName()} was set on fire')
    ...
    def actAsSunPriestPt3(self, target, number, enemies, player):
        if self.lastPtAttackMethodUsed < 3:
            ...
            self.timesSummoned = 0
            play('The Coming Darkness.mp3')
            printWithPause(0.5, '\033[95m', 'The sun priest is changing its strategy.')
            self.newFoes.append(foe('the sun', number, scanned=1))
            number += 1
    ...
    def actAsHelplessSunPriest(self, target, number, enemies, player):
        printWithPause(0.5, '\033[96m', f'The sun priest is begging for your mercy.')

    def getDamage(self, enemies):
        if self.type == 'alien cultist':
            self.attack = self.standardAttack * len([enemy for enemy in enemies if
                                                     enemy.type in ['alien cultist', 'alien devotee']])
